...
from bert_classifier import ClassifierBERTv2
import datetime
...
def tuning(task, num_class, batch_size, epochs, warm_up, lr):
    print(f'Doing {task}...')
    best = 0
    bert = PretrainerBERTv2(num_layers, vocab_size, seq_len, hidden_size, dff, num_heads, dropout_rate)
    optimizer = tfa.optimizers.LAMB(learning_rate=0.00176, weight_decay_rate=0.01)
    checkpoint_path = "./checkpoints/train_v2/ckpt-16"
    ...
    classifier = ClassifierBERTv2(num_class, num_layers, vocab_size, seq_len, hidden_size, dff, num_heads, dropout_rate)
    current_time = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    writer = tf.summary.create_file_writer(f"./{task}/logs_v2_small/" + current_time + '/train')
    ...
    def train(input_ids, label, seg_ids, mask):
        ...
        if task == 'CoLA':
            ...
            prediction = tf.one_hot(prediction, depth=2)
    ...
    for step, data in enumerate(tqdm(dataset)):
        ...
        if (step+1) % 100 == 0:
            ...
            for (test_step, (test_line, test_label, test_s, test_p)) in enumerate(test_dataset):
                ...
                if 'CoLA' not in task and 'STS' not in task:
...
if MRPC:
    best1, f1_score1 = tuning('MRPC', 2, 16, 10, 100, 5e-4)
    print(f1_score1)

    best3, f1_score3 = tuning('MRPC', 2, 16, 10, 200, 3e-4)
    print(f1_score3)

    best4, f1_score4 = tuning('MRPC', 2, 32, 10, 200, 3e-4)
    print(f1_score4)
...
if COLA:
    ...
    best1 = tuning('CoLA', 2, 16, 4, 320, 5e-4)
    matthew.reset_states()
...
if STS:
    ...
    QQP_best = ([best1, best2, best3, best4])

if MNLI:
    best1, best_mis1 = tuning('MNLI', 3, 64, 4, 1000, 8e-4)
    best2, best_mis2 = tuning('MNLI', 3, 64, 4, 1000, 1e-3)
    best3, best_mis3 = tuning('MNLI', 3, 32, 4, 1000, 8e-4)
    best4, best_mis4 = tuning('MNLI', 3, 16, 4, 1000, 8e-4)
...
if QQP:
    best1, f1_score1 = tuning('QQP', 2, 32, 4, 1000, 5e-4)
    print(f1_score1)
    best2, f1_score2 = tuning('QQP', 2, 64, 4, 1000, 3e-4)
    print(f1_score2)
    best3, f1_score3 = tuning('QQP', 2, 16, 4, 1000, 5e-4)
    print(f1_score3)
    best4, f1_score4 = tuning('QQP', 2, 64, 4, 1000, 5e-4)
    print(f1_score4)
